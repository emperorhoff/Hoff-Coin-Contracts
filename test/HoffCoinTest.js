  ...
  describe("Token transfers", function () {
    ...
    it("addresses should have 0 balance", async function () {
      ...
      it("decrements allowance by amount of transferFrom", async function () {
        ...
        const address2Allowance = await hoffCoinToken.allowance(owner.address, address2.address);
        await expect(address2Allowance.toString()).to.equal("1000");
        await hoffCoinToken.connect(address2).transferFrom(owner.address, address3.address, 300);
        await expect((await hoffCoinToken.balanceOf(address3.address)).toString()).to.equal("300");
        await expect((await hoffCoinToken.allowance(owner.address, address2.address)).toString()).to.equal("700");
      });
